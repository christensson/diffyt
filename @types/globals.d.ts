...
export interface YTAppEntity {
  id: string;
  type: "user" | "article" | "ticket" | "issue" | "project" | "app" | string;
  /** Present for issue/article extension points. */
  summary?: string;
  idReadable?: string;
  [key: string]: unknown;
}

export interface YTAppUser {
  id: string;
  login: string;
  name: string;
  avatarUrl?: string;
...
type YTAppInterface = {
  /** UI locale, e.g. "en" or "en_US". */
  locale: string;
  /** The entity hosting the widget, or null/undefined without entity context. */
  entity?: YTAppEntity | null;
  me?: YTAppUser;
